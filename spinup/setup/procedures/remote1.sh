...
chown -R chain_admin:chain_admin /etc/ssh/chain_admin

chmod 755 /etc/ssh/chain_admin

chmod 644 /etc/ssh/chain_admin/authorized_keys

sed -i -e '/^#AuthorizedKeysFile/s/^.*$/AuthorizedKeysFile \/etc\/ssh\/chain_admin\/authorized_keys/' /etc/ssh/sshd_config
...
sh -c 'echo "AllowUsers chain_admin" >> /etc/ssh/sshd_config'
...
sed -i -e '/^Port/s/^.*$/Port 8571/' /etc/ssh/sshd_config

firewall-cmd --add-port 8571/tcp --permanent

sed -i -e '/^Port/s/^.*$/Port 8570/' /etc/ssh/sshd_config

firewall-cmd --add-port 8570/tcp --permanent
...
sh -c 'echo "destemail = chain_admin@gmail.com" >> /etc/fail2ban/jail.local'
...
cat /home/chain_admin/.credentials | chpasswd

rm /home/chain_admin/.credentials
